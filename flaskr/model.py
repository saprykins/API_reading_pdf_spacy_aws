...
class Pdf(Base):
    ...
    named_entities_spacy = Column(String)
    named_entities_aws = Column(String)
...
def save_named_entities_to_string_via_aws(text):
    '''
    Extract named entities from text to a string using AWS service Comprehend
    '''
    list_of_named_entities = []
    string_of_named_entities = ''
    
    # below is AWS named entity recognition

    # initialization of AWS variables
    # and AWS comprehend service
    comprehend = boto3.client(service_name='comprehend', region_name='eu-central-1')
    
    # keeps only first 4900 characters 
    # because of AWS limitations
    text = text[:4000]

    # save found entities in dictionary
    dict = comprehend.detect_entities(Text=text, LanguageCode='en')
    
    # keep only entities, do not keep metadata
    entities = dict["Entities"]

    # keep only person entities
    for entity in entities: 
        if entity["Type"] == 'PERSON':
            list_of_named_entities.append(entity["Text"])

    for entity in list_of_named_entities: 
        string_of_named_entities = string_of_named_entities + entity + ', '
        
    return string_of_named_entities


def save_named_entities_to_string_via_spacy(text):
    '''
    ...
    # below is spacy named entity recognition
...
def save_metadata_and_text_to_data_base(doc_id):
    ...
    string_of_named_entities_from_spacy = save_named_entities_to_string_via_spacy(doc_text_without_references)
    string_of_named_entities_from_aws = save_named_entities_to_string_via_aws(doc_text_without_references)
    ...
    session.add_all(
        [
            Pdf(
                ...
                named_entities_spacy=str(string_of_named_entities_from_spacy),
                named_entities_aws=str(string_of_named_entities_from_aws),
                text=doc_text,
                file_id=doc_id,
            )
        ]
